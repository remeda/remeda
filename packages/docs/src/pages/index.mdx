...
sortBy(USERS, [prop("lastLogin"), "desc"], prop("lastName"), prop("firstName"));
```
...
const withRemeda = map(DATA, (x) => x.toString());
...
pick(obj, ["firstName", "lastName"]); // data-first
pipe(obj, pick(["firstName", "lastName"])); // data-last
```

import { describe, expectTypeOf, test } from "vitest";
import { drop } from "./drop";
import { pipe } from "./pipe";

describe("data-first", () => {
  test("empty array", () => {
    const result = drop([] as [], 2);

    expectTypeOf(result).toEqualTypeOf<[]>();
  });

  test("regular array", () => {
    const result = drop([] as number[], 2);

    expectTypeOf(result).toEqualTypeOf<number[]>();
  });

  test("regular array with union type", () => {
    const result = drop([] as (number | string)[], 2);

    expectTypeOf(result).toEqualTypeOf<(number | string)[]>();
  });

  test("prefixed array", () => {
    const result = drop([1] as [number, ...boolean[]], 2);

    expectTypeOf(result).toEqualTypeOf<boolean[]>();
  });

  test("suffixed array", () => {
    const result = drop([1] as [...boolean[], number], 2);

    expectTypeOf(result).toEqualTypeOf<[...boolean[], number] | []>();
  });

  describe("arrays with a prefix (2) and a suffix (2)", () => {
    test("n === 0 (no drop)", () => {
      const result = drop(
        [1, 2, "a", "b"] as [number, number, ...boolean[], string, string],
        0,
      );

      expectTypeOf(result).toEqualTypeOf<
        [number, number, ...boolean[], string, string]
      >();
    });

    test("n === 1 (drop from the prefix)", () => {
      const result = drop(
        [1, 2, "a", "b"] as [number, number, ...boolean[], string, string],
        1,
      );

      expectTypeOf(result).toEqualTypeOf<
        [number, ...boolean[], string, string]
      >();
    });

    test("n === 2 (remove the prefix)", () => {
      const result = drop(
        [1, 2, "a", "b"] as [number, number, ...boolean[], string, string],
        2,
      );

      expectTypeOf(result).toEqualTypeOf<[...boolean[], string, string]>();
    });

    test("n === 3 (drop the whole prefix, remove from the suffix)", () => {
      const result = drop(
        [1, 2, "a", "b"] as [number, number, ...boolean[], string, string],
        3,
      );

      expectTypeOf(result).toEqualTypeOf<
        [...boolean[], string, string] | [string]
      >();
    });

    test("n === 4 (drop the whole prefix, drop the whole suffix)", () => {
      const result = drop(
        [1, 2, "a", "b"] as [number, number, ...boolean[], string, string],
        4,
      );

      expectTypeOf(result).toEqualTypeOf<
        [...boolean[], string, string] | [] | [string]
      >();
    });

    test("n > 4 (drop more than the constant parts of the array)", () => {
      const result = drop(
        [1, 2, "a", "b"] as [number, number, ...boolean[], string, string],
        5,
      );

      expectTypeOf(result).toEqualTypeOf<
        [...boolean[], string, string] | [] | [string]
      >();
    });
  });

  test("tuple", () => {
    const result = drop([1, "a", true] as const, 2);

    expectTypeOf(result).toEqualTypeOf<[true]>();
  });

  test("union of arrays", () => {
    const result = drop([] as boolean[] | string[], 2);

    expectTypeOf(result).toEqualTypeOf<(boolean | string)[]>();
  });

  test("negative count", () => {
    const result = drop([] as number[], -1);

    expectTypeOf(result).toEqualTypeOf<number[]>();
  });

  test("generalized typed count", () => {
    const result = drop(["a", 1, true] as const, 1 as number);

    expectTypeOf(result).toEqualTypeOf<("a" | 1 | true)[]>();
  });

  test("union of tuples of different length", () => {
    expectTypeOf(
      drop([1, "a"] as [number, string] | [boolean, boolean, number], 1),
    ).toEqualTypeOf<[string] | [boolean, number]>();
  });

  test("dropping more than available", () => {
    expectTypeOf(drop([1, 2, 3] as const, 10)).toEqualTypeOf<[]>();
  });
});

describe("data-last", () => {
  test("empty array", () => {
    const result = pipe([] as [], drop(2));

    expectTypeOf(result).toEqualTypeOf<[]>();
  });

  test("regular array", () => {
    const result = pipe([] as number[], drop(2));

    expectTypeOf(result).toEqualTypeOf<number[]>();
  });

  test("regular array with union type", () => {
    const result = pipe([] as (number | string)[], drop(2));

    expectTypeOf(result).toEqualTypeOf<(number | string)[]>();
  });

  test("prefix array", () => {
    const result = pipe([1] as [number, ...boolean[]], drop(2));

    expectTypeOf(result).toEqualTypeOf<boolean[]>();
  });

  test("suffix array", () => {
    const result = pipe([1] as [...boolean[], number], drop(2));

    expectTypeOf(result).toEqualTypeOf<[...boolean[], number] | []>();
  });

  test("array with suffix and prefix", () => {
    const result = pipe([1, "a"] as [number, ...boolean[], string], drop(2));

    expectTypeOf(result).toEqualTypeOf<[...boolean[], string] | []>();
  });

  test("tuple", () => {
    const result = pipe([1, "a", true] as const, drop(2));

    expectTypeOf(result).toEqualTypeOf<[true]>();
  });

  test("union of arrays", () => {
    const result = pipe([] as boolean[] | string[], drop(2));

    expectTypeOf(result).toEqualTypeOf<(boolean | string)[]>();
  });

  test("negative count", () => {
    const result = pipe([] as number[], drop(-1));

    expectTypeOf(result).toEqualTypeOf<number[]>();
  });

  test("generalized typed count", () => {
    const result = pipe([] as number[], drop(1 as number));

    expectTypeOf(result).toEqualTypeOf<number[]>();
  });
});

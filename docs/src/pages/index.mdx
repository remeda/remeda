...
[`pullObject`](/docs#pullObject), to _complex_ algorithms like
...
_dataFirst_) or with the first parameter, _data_, omitted (what we call
_dataLast_). When a dataLast invocation is detected, Remeda creates a curried
...
R.pick(obj, ["firstName", "lastName"]); // dataFirst
R.pipe(obj, R.pick(["firstName", "lastName"])); // dataLast
```

In the above example, the "dataFirst" approach is more natural and programmer-
...
The "dataLast" approach is helpful when writing data transformations, aka pipes.

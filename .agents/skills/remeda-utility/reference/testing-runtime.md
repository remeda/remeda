...
- A mock used to assert on the lazy `data` argument must either implement all three parameters (`(value, index, data) => ...`) or be a bare `vi.fn()`. An implementation that declares only two parameters reports `length` 2, so `pipe` skips buffering `data` for it and the assertion tests nothing.

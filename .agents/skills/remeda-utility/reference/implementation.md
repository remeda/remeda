...
- Check `type-fest` before writing a new type utility — it covers a lot already. But weigh its instantiation cost (see Type-level performance).
- For boolean logic in conditional types, write explicit nested ternary `extends` checks rather than `And`/`Or` combinators. Two reasons: type-fest's `Or`/`And`/`OrAll`/`AndAll` are very expensive to instantiate (they build on a general `SomeExtend` machine), and (more importantly) any combinator collapses each condition to a `boolean`, so `Combinator<...> extends true ? ... : ...` loses the narrowing that a direct `T extends X ? ... : ...` gives you in its branch (e.g. `T["length"]` only works when `T` was narrowed to an array by the condition itself). Nesting raw `extends` checks keeps that narrowing. For OR, repeat the true-branch (extract it to a named type if it's complex); for AND, nest. Also inline thin type-fest predicates with a trivial one-line equivalent (e.g. `IsUnknown<T>` as `unknown extends T` once `any` is excluded) — it drops an import at no readability cost. But this is about brevity, _not_ narrowing: inlining only recovers narrowing when the raw check would expose a **naked type parameter** (or `infer`) on the left of `extends` (`T extends string ? ...` narrows `T`; `IsString<T> extends true ? ...` does not). A **boxed** check like `[X] extends [never]` never narrows `X` either way, so inlining `IsNever` is purely cosmetic — and since the `[...]` boxing is a non-obvious "stop distribution" idiom, prefer keeping `IsNever` named.

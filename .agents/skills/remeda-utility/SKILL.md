...
- **Benchmarks** (`functionName.bench.ts`) — vitest bench. Only where a performance claim needs backing. When adding or reading one, read `reference/benchmarking.md`: benchmarks run after a shared pollution pass, and which statistic you read changes the answer.

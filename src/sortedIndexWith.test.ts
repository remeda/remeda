...
describe('runtime correctness', () => {
  test('empty array', () => {
    expect(sortedIndexWith([], () => true)).toBe(0);
  });

  test('single value, always false', () => {
    expect(sortedIndexWith([1], () => false)).toBe(0);
  });

  test('single value, always true', () => {
    expect(sortedIndexWith([1], () => true)).toBe(1);
  });

  test('multiple values, always false', () => {
    expect(sortedIndexWith([1, 2, 3, 4, 5], () => false)).toBe(0);
  });

  test('multiple values, always true', () => {
    expect(sortedIndexWith([1, 2, 3, 4, 5], () => true)).toBe(5);
  });
...
function indicesSeen(
  ...
  sortedIndexWith.indexed(items, (pivot, index) => {

...
/**
 * Wraps `func` with a debouncer object that "debounces" (delays) invocations of the function during a defined cool-down period (`waitMs`). The debouncer can be configured to invoke the function either at the start of the cool-down period, the end of it, or at both ends. It can also be configured to allow invocations during the cool-down period (`maxWaitMs`). The debouncer stores the latest arguments provided to the call function so they could be used at the end of the cool-down period when invoking `func` (if configured to invoke the function at the end of the cool-down period).
 * The debouncer stores the value returned by `func` whenever it's invoked. This value is returned on every call, and is accessible via the `cachedValue` property of the debouncer. It's important to note that the value might be different from the value that would be returned from running `func` with the current arguments as it's a cached value from a previous invocation.
